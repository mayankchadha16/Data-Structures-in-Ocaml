...
* On the dataset of 1 million random numbers, we found that Red-Black Trees took 4 seconds to insert them, while Binary Search Trees took 2.9 seconds. This suggests that their performance on random datasets is comparable.
* However, on the ordered dataset of natural numbers up to 0.1 million, Red-Black Trees outperformed Binary Search Trees significantly. Red-Black Trees took only 0.19 seconds to insert them, while Binary Search Trees took 29 seconds. These results align with the theoretical expectations for these data structures.
